...
VaselineAd.prototype.renderFirstScreen = function () {
	var self = this;
  	...
  	$('#dark-body').on('mousedown', function(){
  		clearTimeout(self.autoTimeout);
  		self.lighten();
  		self.whiten();
  	});
  	$('#dark-body, #rma-widget').on('touchstart', function(){
  		clearTimeout(self.autoTimeout);
  		self.lighten();
  		self.whiten();
  	});
  	$('#dark-body, #rma-widget').on('click', function(){
  		clearTimeout(self.autoTimeout);
  		self.lighten();
  		self.whiten();
  	});
  	...
  	self.autoTimeout =  setTimeout(function(){
  		self.animateToSecondScreen();
  	}, 5000);

};

VaselineAd.prototype.animateToSecondScreen = function () {
  	var self = this;
	...
	$('#dark-body, #rma-widget').off('click');
	$('#dark-body, #rma-widget').off('mousedown');
	$('#dark-body, #rma-widget').off('touchstart');

	$('#first-screen').append(hand);
	$('#dark-body').on('mousedown', self.whiten);
	$('#dark-body, #rma-widget').on('touchstart', self.whiten);
	$('#dark-body, #rma-widget').on('click', self.whiten);
};

VaselineAd.prototype.lighten = function(){
	var self = this;
	$('#light-body').attr('src', self.sdk.path + 'img/light-body.png');
	$('#light-body').addClass('fade-in');
}


VaselineAd.prototype.whiten = function(){
  	var sdk = new mads();
  	var eventsBound = false;

  	sdk.tracker('E', 'vv_wipe');
  	$('#hand').remove();
  	$('#dark-body').eraser('clear');
  	$('#dark-body, #rma-widget').off('click');
	$('#dark-body, #rma-widget').off('mousedown');
	$('#dark-body, #rma-widget').off('touchstart');
  	var canvas = $('canvas');
	...
	bindEvents();
  	...
  	head.animate({ opacity: 1 }, 5000, function() {
  	...
  	$('#dark-body').animate({ opacity: 0 }, 500, null);
